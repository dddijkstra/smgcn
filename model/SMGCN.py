...
class SMGCN(nn.Module):
    def __init__(self, data_config, pretrain_data):
        ...
        *********************************************************
        ...
        self.hypergraph_emb = torch.tensor(
            np.load("data/Herb/hypergraph_node_embeddings.npy"), dtype=torch.float32
        ).to(self.device)
    ...
    def forward(self, users, user_set=None, pos_items=None, train=True):
        ...
        if self.alg_type in ['SMGCN']:
            if train:
                ...
                # 用户三向量两两对比
                cl_loss_user_1 = self.compute_contrastive_loss(
                    u_g_embeddings, user_pair_embeddings)
                cl_loss_user_2 = self.compute_contrastive_loss(
                    u_g_embeddings, hyper_user_emb)
                cl_loss_user_3 = self.compute_contrastive_loss(
                    user_pair_embeddings, hyper_user_emb)
                cl_loss_user_total = cl_loss_user_1 + cl_loss_user_2 + cl_loss_user_3

                # 物品三向量两两对比
                cl_loss_item_1 = self.compute_contrastive_loss(
                    i_g_embeddings, item_pair_embeddings)
                cl_loss_item_2 = self.compute_contrastive_loss(
                    i_g_embeddings, hyper_item_emb)
                cl_loss_item_3 = self.compute_contrastive_loss(
                    item_pair_embeddings, hyper_item_emb)
                cl_loss_item_total = cl_loss_item_1 + cl_loss_item_2 + cl_loss_item_3
                ...
                return user_embeddings, all_user_embeddins, i_g_embeddings, cl_loss_user_total, cl_loss_item_total
            else:
                # 测试阶段不需要计算对比损失
