...
# 加载节点映射
herb_map = load_mapping(root_path + "/herb_mapping.txt")
sym_map = load_mapping(root_path + "/sym_mapping.txt")
...
# 导入症状-草药关系
with open(root_path + "/train.txt", encoding="utf-8") as f:
    for line in f:
        if "\t" not in line:
            continue
...
# 导入症状-症状相似关系
with open(root_path + "/symPair-5.txt", encoding="utf-8") as f:
    for line in f:
...
# 导入草药-草药相似关系
with open(root_path + "/herbPair-40.txt", encoding="utf-8") as f:
    for line in f:
